...
const (
	UserName     string = "o-ran"
	Password     string = "password"
	Addr         string = "140.113.144.124"
	Port         int    = 13306
	Database     string = "oran"
	MaxLifetime  int    = 0
	MaxOpenConns int    = 50
	MaxIdleConns int    = 50
)
...
func Add_ue_info(DB *sql.DB, ue UeMetricsEntry) {
	var ueid string
	var ueid2 string
	var ueid3 string
	row := DB.QueryRow("select UEID from ue_info where UEID=?",ue.UeID)
	row.Scan(&ueid)
	if ueid != "" {
		DB.Exec("update ue_info set ServingCellID=?, UEPDCPBytesDL=?, UEPDCPBytesUL=?, UEPRBUsageDL=?, UEPRBUsageUL=?, MeasPeriodRF=?, MeasPeriodUEPDCPBytes=?,  MeasPeriodUEPRBUsage=? where UEID=?", ue.ServingCellID, ue.PDCPBytesDL, ue.PDCPBytesUL, ue.PRBUsageDL, ue.PRBUsageUL, ue.MeasPeriodRF, ue.MeasPeriodPDCP, ue.MeasPeriodPRB,  ue.UeID)
                //if err != nil {
		...
		DB.Exec("insert INTO ue_info(UEID, ServingCellID, UEPDCPBytesDL, UEPDCPBytesUL, UEPRBUsageDL, UEPRBUsageUL, MeasPeriodRF, MeasPeriodUEPDCPBytes,  MeasPeriodUEPRBUsage) values(?,?,?,?,?,?,?,?,?)", ue.UeID, ue.ServingCellID, ue.PDCPBytesDL, ue.PDCPBytesUL, ue.PRBUsageDL, ue.PRBUsageUL, ue.MeasPeriodRF, ue.MeasPeriodPDCP, ue.MeasPeriodPRB)
		//if err != nil {
	...
	row2 := DB.QueryRow("select UEID from ue_meas where UEID=? and CellID=?", ue.UeID, ue.ServingCellID)
        row2.Scan(&ueid2)
        if ueid2 != "" {
	        DB.Exec("update ue_meas set rsrp=?, rsrq=?, rssinr=?, serving=?, tv_sec=?, tv_nsec=? where UEID=? and CellID=? ",ue.ServingCellRF.RSRP, ue.ServingCellRF.RSRQ, ue.ServingCellRF.RSSINR, 1, time.Now().Unix(), time.Now().UnixNano(), ue.UeID, ue.ServingCellID)
                DB.Exec("update ue_meas set serving=?, tv_sec=?, tv_nsec=? where UEID=? and CellID!=? ", 0, time.Now().Unix(), time.Now().UnixNano(), ue.UeID, ue.ServingCellID)
		//if err != nil {
		//	fmt.Printf("Update data failed,err:%v", err)
                //        return
                //}
                //fmt.Println("update ServingCell data successd:", result)
                ...
                DB.Exec("insert INTO ue_meas(UEID, CellID, rsrp, rsrq, rssinr, serving, tv_sec, tv_nsec) values(?,?,?,?,?,?,?,?)", ue.UeID, ue.ServingCellID, ue.ServingCellRF.RSRP, ue.ServingCellRF.RSRQ, ue.ServingCellRF.RSSINR, 1, time.Now().Unix(), time.Now().UnixNano())
		//if err != nil {
                //        fmt.Printf("Insert data failed,err:%v", err)
                //        return
                //}
                ...
                //        return
                //}
                //fmt.Println("Insert ServingCell data id:", lastInsertID)
	...
	//fmt.Println("length", len(ue.NeighborCellsRF))
	for i :=0; i < len(ue.NeighborCellsRF); i++ {
		row3 := DB.QueryRow("select UEID from ue_meas where UEID=? and CellID=?", ue.UeID, ue.NeighborCellsRF[i].CellID)
	        row3.Scan(&ueid3)
		if ueid3 != "" {
			DB.Exec("update ue_meas set rsrp=?, rsrq=?, rssinr=?, serving=?, tv_sec=?, tv_nsec=? where UEID=? and CellID=? ",ue.NeighborCellsRF[i].CellRF.RSRP, ue.NeighborCellsRF[i].CellRF.RSRQ, ue.NeighborCellsRF[i].CellRF.RSSINR, 0, time.Now().Unix(), time.Now().UnixNano(), ue.UeID, ue.NeighborCellsRF[i].CellID)
			//if err != nil {
			//	fmt.Printf("Update data failed,err:%v", err)
			//	return
			//}
	                //fmt.Println("update Neighbor data successd:", result)
	                ...
	                DB.Exec("insert INTO ue_meas(UEID, CellID, rsrp, rsrq, rssinr, serving, tv_sec, tv_nsec) values(?,?,?,?,?,?,?,?)", ue.UeID, ue.NeighborCellsRF[i].CellID, ue.NeighborCellsRF[i].CellRF.RSRP, ue.NeighborCellsRF[i].CellRF.RSRQ, ue.NeighborCellsRF[i].CellRF.RSSINR, 0, time.Now().Unix(), time.Now().UnixNano())
	                //if err != nil {
	                //        fmt.Printf("Insert data failed,err:%v", err)
	                //        return
	                //}
	                ...
	                //        return
	                //}
	                //fmt.Println("Insert Neighbor data id:", lastInsertID)
...
func Add_cell_info(DB *sql.DB, cell CellMetricsEntry) {
	var cellid string
        row4 := DB.QueryRow("select CellID from cell_info where CellID=?", cell.CellID)
        row4.Scan(&cellid)
        if cellid != "" {
                DB.Exec("update cell_info set PDCPBytesDL=?, PDCPBytesUL=?, AvailPRBDL=?, AvailPRBUL=?, MeasPeriodPDCPBytes=?, MeasPeriodAvailPRB=?, tv_sec=?,  tv_nsec=? where CellID=?", cell.PDCPBytesDL, cell.PDCPBytesUL, cell.AvailPRBDL, cell.AvailPRBUL, cell.MeasPeriodPDCP, cell.MeasPeriodPRB, time.Now().Unix(), time.Now().UnixNano(), cell.CellID)
                //if err != nil {
                //        fmt.Printf("Update data failed,err:%v", err)
                //        return
                //}
                //fmt.Println("update cell_info data successd:", result)
                ...
                DB.Exec("insert INTO cell_info(CellID, PDCPBytesDL, PDCPBytesUL, AvailPRBDL, AvailPRBUL, MeasPeriodPDCPBytes, MeasPeriodAvailPRB, tv_sec,  tv_nsec) values(?,?,?,?,?,?,?,?,?)", cell.CellID, cell.PDCPBytesDL, cell.PDCPBytesUL, cell.AvailPRBDL, cell.AvailPRBUL, cell.MeasPeriodPDCP, cell.MeasPeriodPRB, time.Now().Unix(), time.Now().UnixNano())
                //if err != nil {
                //        fmt.Printf("Insert data failed,err:%v", err)
                //        return
                //}
                ...
                //        return
                //}
                //fmt.Println("Insert cell_info data id:", lastInsertID)
				...
				if containerType == 1 {
					...
					for j := 0; j < oDUUE.CellResourceReportItemCount; j++ {
						...
						for k := 0; k < cellResourceReportItem.UeResourceReportItemCount; k++ {
							...
							//add
							log.Printf("=======================================================================")
                                                        log.Printf("Get TS-UE-Metrics:")
                                                        log.Printf(ueJsonStr)
                                                        log.Printf("=======================================================================")
					...
					for j := 0; j < oCUCPUE.CellResourceReportItemCount; j++ {
						...
						for k := 0; k < cellResourceReportItem.UeResourceReportItemCount; k++ {
							...
							//add
                                                        log.Printf("=======================================================================")
                                                        log.Printf("Set new UE-Metrics:")
                                                        log.Printf(string(newUeJsonStr))
                                                        log.Printf("=======================================================================")
			...
			if flag {
				...
				if err != nil {
					...
					if retStr["{TS-cell-metrics}," + cellIDHdr] != nil {
						...
						//add
                                                        log.Printf("=======================================================================")
                                                        log.Printf("Get cellMetrics:")
                                                        log.Printf(cellJsonStr)
                                                        log.Printf("=======================================================================")
				...
				//add
				Add_cell_info(DB, cellMetrics)
                                log.Printf("=======================================================================")
                                log.Printf("Set newCellMetrics:")
                                log.Printf(string(newCellJsonStr))
                                log.Printf("=======================================================================")
