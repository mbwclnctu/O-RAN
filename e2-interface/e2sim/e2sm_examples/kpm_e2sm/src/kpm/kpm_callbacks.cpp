  ...
  while ( getline(input, str) ) {
	...
	for (int i = 0; i < numMeasReports; i++) {
	  ...
	  if (ueId.find("Pedestrian") != string::npos) {
	    ...
	    crnti_buf[1] = indval/9 + 1;
	    crnti_buf[0] = indval%9 + 1;
	    fprintf(stderr, "=====indval:%d  %d  ",indval/9+1, indval%9+1);
/*	    if (indval < 10) {
	      crnti_buf[1] = indval;
	      crnti_buf[0] = 0;
	    } else {
...
*/	    
	  ...
	  for (int j = 0; j < numNeighborCells; j++) {
	    ...
	    neighbor_str += "{\"CID\" : \"" + std::string(cid_buf) + "\", \"CellRF\" : {\"rsrp\": " + std::to_string(nextNbRsrp) +
