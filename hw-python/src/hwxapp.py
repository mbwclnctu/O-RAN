...
class HWXapp:
    ...
    def _post_init(self, rmr_xapp):
        ...
        """
        ue_data_list = list(rmr_xapp.sdl.find_and_get(UE_NS, "", usemsgpack=False).values())
        print(ue_data_list)
       ...
       #sub_mgr = SubscriptionManager(rmr_xapp)
        ...
        #jstr = json.loads(ue_data_list)
        #print(jstr)
        for i in range(100):
            ue_data_list = rmr_xapp.sdl_find_and_get("kpimon", "", usemsgpack=False)
            for k in ue_data_list.keys():
                j=json.loads(ue_data_list[k])
                print(j)
            time.sleep(5)
